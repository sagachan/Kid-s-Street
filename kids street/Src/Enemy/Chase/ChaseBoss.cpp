...
namespace
{
	...
	const float		FIND_AREA = 300.0f;							//プレイヤーを見つけることができる警戒エリアの半径
	...
	const int		BOSS_ARRIVAL = 5;							//ボスが遠距離でプレイヤーを追いかけるときにプレイヤーに追いつくまでの時間
	const int		BOSS_SERCH_TIME = 2;						//ボスがプレイヤーを見失ってから索敵にかける時間
...
void CChaseBoss::ReturnRoute()
{
	...
	////線分の当たり判定を使って、目的地とボスの間に障害物がないかどうかを判定
	//MV1_COLL_RESULT_POLY colInfo = MV1CollCheck_Line(colMapHndl_, -1, pos_, targetPos);
