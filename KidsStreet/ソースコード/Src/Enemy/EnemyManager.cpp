...
{													
	...
	const VECTOR	ENEMY_CAGE_POS1 = VGet(-1460.0f, 0.0f, 1660.0f);
	const VECTOR	ENEMY_CAGE_POS2 = VGet(-1460.0f, 0.0f, 1750.0f);
	const float     BOSS_START_POS_Y = -1.0f;
	const float		BOSS_START_POS_Z = 5.0f;
}
...
void CEnemyManager::Set()
{
	...
	bossInterPos.y = BOSS_START_POS_Y;
	bossInterPos.z -= BOSS_START_POS_Z;
...
void CEnemyManager::Step()
{
	...
	//ボス戦開始条件
	if (ENEMY_CAGE_POS1.x > playerPos.x &&
		ENEMY_CAGE_POS1.z < playerPos.z &&
		ENEMY_CAGE_POS2.z > playerPos.z &&
		boss_->GetHP() == BOSS_MAX_HP &&
		Sceneflg_ == true)
	{
...
void CEnemyManager::Fin()
{
	...
	CSaveScene::GetInstance()->SetBossFlg(Sceneflg_);
}
