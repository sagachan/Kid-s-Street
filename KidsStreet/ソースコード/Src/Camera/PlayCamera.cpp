...
namespace
{
	...
	//カメラのX回転値最大
	const float PLAY_CAMERA_ROTATION_MAX_X = (70.0f * DX_PI_F / 180.0f);
	//カメラのX回転値最小
	const float PLAY_CAMERA_ROTATION_MIN_X = (-40.0f * DX_PI_F / 180.0f);
	//コンティニュー時の回転
	const float PLAY_CAMERA_CONT = 4.8f;
...
void CPlayCamera::Init(VECTOR pos, VECTOR look, VECTOR up)
{
	...
	if (!CSaveScene::GetInstance()->GetBossFlg())
	{
		cameraInfo_.fRotY_ = PLAY_CAMERA_CONT;
	}
}
...
void CPlayCamera::Step()
{
	...
	if (PLAY_CAMERA_ROTATION_MAX_X <= cameraInfo_.fRotX_){
		cameraInfo_.fRotX_ = PLAY_CAMERA_ROTATION_MAX_X;
	}
	if (PLAY_CAMERA_ROTATION_MIN_X >= cameraInfo_.fRotX_){
		cameraInfo_.fRotX_ = PLAY_CAMERA_ROTATION_MIN_X;
	}
